  ...
  return (
    <div className="flex h-screen bg-[var(--background)] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col ml-14 overflow-hidden">
        <header className="flex items-center justify-between px-8 h-16 border-b border-[var(--border)] bg-[var(--background)] flex-shrink-0 z-10 relative">
          <h1 className="text-[16px] font-semibold text-[var(--primary-text)] flex items-center gap-2">
            <FolderOpen size={18} className="text-[var(--secondary-text)]" />
            File Explorer
          </h1>
              ...
              <>
                ...
                <div className="relative">
                  <button 
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 rounded-[8px] border border-[var(--border)] bg-[var(--surface)] text-[13px] text-[var(--primary-text)] hover:border-[var(--border-hover)] transition-colors min-w-[200px]"
                  >
                  ...
                  {showRepoDropdown && (
                    <>
                      ...
                      <div className="absolute top-full mt-2 right-0 w-[240px] bg-[var(--surface)] border border-[var(--border)] rounded-xl overflow-hidden shadow-2xl z-20 max-h-[300px] overflow-y-auto p-1">
                        {repositories.map(repo => (
                          <button
                            key={repo.id}
                            onClick={() => handleSelectRepo(repo.id)}
                            className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-[13px] text-left transition-colors ${
                              repo.id === selectedRepoId ? "bg-[var(--border)] text-[var(--primary-text)]" : "text-[var(--secondary-text)] hover:bg-[var(--surface-hover)] hover:text-[var(--primary-text)]"
                            }`}
                          >
                            <Folder size={14} className={repo.id === selectedRepoId ? "text-blue-400" : "text-[var(--muted-text)]"} />
        ...
        <main className="flex-1 overflow-hidden p-8 flex justify-center">
          {isLoading ? (
            ...
            <div className="flex items-center justify-center h-full text-center text-[var(--secondary-text)] text-[14px]">
