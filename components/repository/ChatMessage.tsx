...
function renderContent(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    if (part.startsWith("**") && part.endsWith("**")) {
      return <strong key={i} className="font-semibold text-[var(--primary-text)]">{part.slice(2, -2)}</strong>;
    }
    if (part.startsWith("`") && part.endsWith("`")) {
...
export default function ChatMessage({ message, onSourceClick }: ChatMessageProps) {
  if (message.role === "user") {
    return (
      <div className="flex items-start gap-3 animate-fade-in">
        {/* Avatar */}
        <div className="w-7 h-7 rounded-full bg-[var(--border)] border border-[var(--border-hover)] flex items-center justify-center text-[11px] font-medium text-[var(--secondary-text)] flex-shrink-0 mt-0.5">
          O
        </div>
        <div>
          <p className="text-[12px] text-[var(--muted-text)] mb-1 font-medium">You</p>
          <h2 className="text-[22px] font-semibold text-[var(--primary-text)] leading-tight">
            {message.content}
          </h2>
        </div>
      </div>
  ...
  return (
    <div className="flex items-start gap-3 animate-slide-up">
      {/* AI Avatar */}
      <div className="w-7 h-7 rounded-[6px] bg-[var(--surface)] border border-[var(--border)] flex items-center justify-center flex-shrink-0 mt-0.5">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[12px] text-[var(--muted-text)] mb-3 font-medium">CodeBase AI</p>

        {/* Sections */}
        {message.sections?.map((section, idx) => (
          <div key={idx} className={cn("mb-6", idx > 0 && "")}>
            <p className="text-[11px] uppercase tracking-widest text-[var(--muted-text)] mb-2 font-medium">
              {section.title}
            </p>

            {section.content && (
              <p className="text-[14px] text-[var(--code-text)] leading-relaxed">
              ...
              <ul className="space-y-3 mt-2">
                {section.items.map((item, iIdx) => (
                  <li key={iIdx} className="flex items-start gap-2.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-[var(--border-hover)] flex-shrink-0 mt-2" />
                    <span className="text-[14px] text-[var(--code-text)] leading-relaxed">
                      {renderContent(item)}
                    </span>
                  </li>
                ))}
        ...
        {message.sources && message.sources.length > 0 && (
          <div className="mt-4">
            <p className="text-[11px] uppercase tracking-widest text-[var(--muted-text)] mb-2.5 font-medium">
              Key Files
            </p>
        ...
        <div className="mt-6 flex items-center gap-3 border-t border-[var(--border)] pt-4">
          <button 
            ...
            className="flex items-center gap-1.5 text-[12px] text-[var(--secondary-text)] hover:text-[var(--primary-text)] transition-colors"
          >
